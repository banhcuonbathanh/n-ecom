...
Go Gin · JWT · bcrypt · Redis · MySQL  ·  v1.3  ·  Thang 4 / 2026 |
| --- |

| Pham vi tai lieu nay: Giai thich day du bang ca van ban va so do truc quan toan bo Auth Module — bao gom 4 data flows chinh (Login, Request Auth, Refresh, Logout), he thong RBAC 6 roles, 9 case studies co that, va QR Guest Session design (v1.3). Moi section bao gom 'tai sao' (why), 'cai gi' (what), va 'nhu the nao' (how). |
| --- |
...
Tach khoi §0.1 (responsibilities) de de scan. Dung khi can tim nhanh file can sua.
| File | Layer | Owner / Ghi Chu |
| --- | --- | --- |
| pkg/jwt/jwt.go | pkg/ | GenerateAccessToken, ParseToken, GenerateGuestToken (v1.3) |
| pkg/bcrypt/bcrypt.go | pkg/ | HashPassword cost=12, ComparePassword |
| pkg/crypto/hash.go | pkg/ | [NEW v1.1] SHA256 hash chung — thay code trung lap |
| internal/repository/auth_repo.go | repository/ | CRUD sessions, rate limit Redis, TX-aware |
| internal/service/auth_service.go | service/ | Login, Refresh, Logout, Me, ValidateAccessToken |
| internal/middleware/auth.go | middleware/ | AuthRequired — extract Bearer, inject claims |
| internal/middleware/rbac.go | middleware/ | RequireRole, AtLeast, RequireOwner, CustomerOrStaff |
| internal/handler/auth_handler.go | handler/ | 4 HTTP handlers + RegisterRoutes + Cookie mgmt |
...
| v1.3 CHANGES: Them Section 0.3 (File Structure tach rieng), Section 8 (Open Questions). Sections 0-4 khong thay doi tu v1.2. v1.2: Section 5 QR Guest Session. Giai quyet bai toan: anonymous customer quet QR tren ban, dat mon khong can login, khong can tao account. |
| --- |
...
# Section 8 — Open Questions & Known Limitations
*v1.3 NEW — Consolidate cac van de chua resolve tu cac sections khac. Moi issue can duoc BA/Lead arbitrate truoc khi implement.*
| Issue | Mo Ta & Impact | Nguon | Status |
| --- | --- | --- | --- |
| OQ-001: Customer role ambiguity | staff.role ENUM co 'customer' nhung online/QR customers la anonymous — khong co staff account. Impact: RequireRole() middleware chua ro phai handle 'customer' the nao. Recommendation: Remove 'customer' khoi ENUM, dung Guest JWT (table-scoped) la du. Phai clarify voi BA truoc khi implement Staff module. | 001_auth.sql NOTE + MASTER §3 | ⚠️ OPEN — can BA arbitrate |
| OQ-002: Diagram render dependency | Section 0.1 va cac 'Hinh 1.x' reference diagrams embedded duoi dang anh. Neu doc chi co text (extract-text, API) thi mat visual context cho Hinh 1.1–1.4, 2.1, 4.1, 5.1, 6.1. Recommendation: Export diagrams sang separate PNG files hoac them alt-text mo ta tu dong cho tung hinh. | Section 0–6 (tat ca so do) | ℹ️ MINOR — track only |
| OQ-003: Fail-open khi Redis down | Khi Redis down, ValidateAccessToken() chap nhan token (fail-open). Day la intentional design — tranh Redis outage ha sap auth. Nhung neu attacker chu dong down Redis truoc khi logout token bi an cap, blacklist khong work. Risk: accepted vi DB session row la hard guarantee (attacker van khong tao duoc token moi). Document ro trong Section 4. | Section 4 — Security + Logout Flow | ℹ️ ACCEPTED RISK |
| OQ-004: Guest JWT mat khi reload | Guest JWT luu trong Zustand RAM — mat khi user reload trang. User phai quet QR lai. Trade-off chap nhan duoc cho guest session (TTL 4h, simple). Tuy nhien neu Phase 2 them pre-booking flow, guest session co the can persist. Flag de review khi Phase 2 bat dau. | Section 5 — QR Guest Session | 📋 PHASE 2 REVIEW |

| BanhCuon System  ·  Auth Module  ·  Data Flow & Case Study — Full Explanation with Visual Diagrams  ·  v1.3  ·  Thang 4/2026 |
| --- |
