...
flowchart TD
    ...
    D -- Invalid / expired --> E[QR expired / invalid → Show error: 'Mã QR không hợp lệ'\nPrompt customer to re-scan QR]
...
```mermaid
flowchart TD
    QR([Scan QR code on table]) --> AUTH{JWT authentication\nsuccessful?}
    AUTH -- NO --> QR_INVALID[QR expired / invalid]
    QR_INVALID --> SCAN_AGAIN[Show message:\nScan QR again]
    AUTH -- YES --> MENU[Browse menu:\nAll / Bánh Cuốn / Chả / Combo]

    MENU --> CARD[Select product card\n→ check availability in real-time]
    CARD -- NO / unavailable --> BADGE[Show 'Hết hàng' badge — no action]
    CARD -- available --> HAS_TOP{Has toppings\nor combo?}
    HAS_TOP -- Toppings --> TOP_MODAL[Topping selection modal]
    HAS_TOP -- Combo --> COMBO_MODAL[Combo modal]
    TOP_MODAL --> ADD_CART[Add to cart]
    COMBO_MODAL --> ADD_CART

    ADD_CART --> CART[Cart drawer\nwith +/- quantity adjusters]

    CART --> CHECKOUT[Checkout form:\nName / Phone /\nPayment method\nCash · MoMo · ZaloPay]
    CHECKOUT --> VALIDATE{Zod validation}
    VALIDATE -- Fail\nInvalid phone / missing fields --> ERR[Show inline errors]
    ERR --> CONFLICT[409 Conflict:\nTable already has\nactive order]
    CONFLICT --> TOAST[⚠ Toast notification]
    VALIDATE -- Pass --> POST_ORDER[POST /api/v1/orders]
    POST_ORDER --> ORDER_CREATED[201 Created → clear cartStore]
    ORDER_CREATED --> TRACK[Redirect /order/orderId]

    TRACK --> SSE[Order tracking via\nSSE real-time updates]
    SSE --> DISPLAY[Show order details\nProgress bar · Item status dots · Total]

    %% SSE disconnection handling
    SSE -. connection lost .-> DISC_TOAST[⚠ Disconnection toast]
    DISC_TOAST --> DISC_BANNER[Show disconnected banner]
    DISC_BANNER -. reconnect .-> SSE
    ...
    EVENTS -- order_completed --> DONE([Order completion])

    %% Error states during cart / checkout
    CART -. item removed\nwhile in cart .-> ITEM_REMOVED[Item removed from\nmenu while in cart]
    CHECKOUT -. payment issue .-> PAY_FAIL[Payment failed /\ntimeout / cancelled]
    PAY_FAIL --> RETRY[Retry or\nchange method]
    CART -. stock issue .-> OOS[Item out of stock /\nunavailable]
    OOS --> ALT[Show alternative or\ndisable button]

    %% Cancel request flow
    DISPLAY --> CANCEL_REQ{Cancel\nrequest?}
    CANCEL_REQ -- YES --> PROG{Progress < 30%?\nif NO}
    PROG -- YES --> FULL_REFUND[Cancel whole order\n→ Full refund]
    PROG -- NO --> STAFF_APPROVAL[Requires staff\napproval]
    STAFF_APPROVAL --> PARTIAL[Partial refund possible]
    CANCEL_REQ -- add more items --> ADD_MORE[Add more items\n→ Redirect to menu]
    ADD_MORE --> NEW_PAY[New payment for\nadditional items]
```
