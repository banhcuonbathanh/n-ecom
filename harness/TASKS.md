...
| F-10 | BE engineering playbook from `reference/docs/be` — adopt the reusable build rules (`harness/BE_PLAYBOOK.md` + `harness/diagrams/task-F-10.html`) | F-1, F-8 | Playbook covers goose+sqlc data-layer workflow, migration-file standard, Go/Gin gotcha rules, caching discipline adds, BE build order, seed/smoke tooling rule, code-summary doc rule; zero duplication with BE_STATE/ARCHITECTURE (links instead); task page renders both themes | 🔄 | |
