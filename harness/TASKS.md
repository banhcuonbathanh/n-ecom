...
| F-15 | Customer-menu page build plan from `reference/…/customer/customer_menu` — consolidated FE+BE plan in ONE folder (`harness/plans/customer_menu/`: `PLAN.md` + `plan.html`) reconciled with OVERALL_PLAN C/T-phase rows + FE_STATE/BE_STATE rules | F-9, F-12, F-11 | Folder holds one MD plan (FE + BE + API contract + task breakdown mapped to TASKS.md rows) and one HTML plan page rendering both themes covering FE and BE; zero rule duplication (links to owning docs); indexes updated per Hard Rule 6 | ⬜ | |
