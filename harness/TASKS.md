...
| F-17 | Customer order-detail page build plan (the live SSE order view `/order/:id`) from `reference/…/customer/customer_order_detail` — 4-doc set per `PAGE_PLAN_GUIDE.md` in `harness/plans/customer_order_detail/` (`_PLAN.md` + `_plan.html` + `_how-it-works.html` + `_mockup-1.html`) reconciled with OVERALL_PLAN O/R-phase rows, the §3.5 realtime design, DB_SCHEMA order tables, and FE_STATE/BE_STATE rules | F-15, F-16, F-9 | Folder holds the MD plan (FE+BE, 5-endpoint contract incl. SSE, behavior spec, TASKS-row mapping, defects designed out) + 3 HTML companions rendering both themes; every reference SSE bug mapped to a §3.5 countermeasure; zero rule duplication (links owning docs); indexes updated per Hard Rule 6 | 🔄 | |
