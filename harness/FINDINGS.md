...
| F43 | 2026-07-24 | chat | ⚠️ | undesigned-dead-end | **The QR-only ruling ([F39](#)) creates an unspecified state:** a guest who opens `/menu` without scanning (direct link, bookmark, browsing before sitting) can browse but has no way to order — and menu `PLAN §4.4` #8's answer for "no table" was *navigate to `/checkout`*, a page v1 no longer builds. Left as-is that is a dead button or a 404, i.e. the reference's own "silently broken" class | 🔵 open | Plan default: "Thanh toán" with no table opens a **quét-mã-QR prompt** (VN copy in `menu.vn.ts`, one modal, no route) — the cart is kept, so scanning then returning still works. Build it in **C-4b/T-2**; close against the T-2 screenshot | [Session 22 (F-34)](STATE.md) | [PLAN §4.4 #8 note](plans/customer_menu/customer_menu_PLAN.md) |
| F39 | 2026-07-24 | F-34 | ⚠️ | unowned-route | **`/checkout` has no owner.** The menu's second checkout path (`source:'online'` + name/phone, menu `PLAN §4.4` #8) routes to `/checkout`, which `FE_PLAN §1` names *out of scope* — and no other plan folder claims it. So the **online** ordering path cannot complete even after `O-0F` ships | ⛔ won't-fix (v1) | **Owner ruling 2026-07-24:** *"checkout is for shipping; we eat at the restaurant — no checkout page."* v1 is **QR dine-in only** — no `/checkout`, no online order, and no table-less mint (`POST /auth/guest/online` drops out of T-1, leaving one mint). Dated notes written into menu `PLAN §1/§3.1/§4.4 #8`, `BE_PLAN §1`, `FE_PLAN §1`; schema untouched so the path can be switched on later without a migration. Consequence tracked as [F43](#) | [Session 22 (F-34)](STATE.md) | [PLAN §1 ruling note](plans/customer_menu/customer_menu_PLAN.md) |
